...
def get_token_from_header(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> Optional[str]:
    ...
    # Method 2: Try direct header extraction (fallback for edge cases)
    # This is handled by Request object if needed, but HTTPBearer should work
    if not token:
        print("⚠️ No credentials found in Authorization header via HTTPBearer")
